...
``_readout_cache[3]`` and ``[7]``) per node, then 3-STAT pooling per graph
(max + mean + std concatenated). Drops sum, which scales with graph size and
hurt the 4-stat variant on caco2 / bbb_martins.

Std is computed from the variance formula (E[X²] - E[X]²) since PyG doesn't
ship a ``global_std_pool``.

Fingerprint dim = 2 layers * 96 hidden * 3 stats = 576-d.
Verbatim MiniMol HPs (no hidden_dim override).

Output: results/admet_layers48_mms_oneshot.csv
"""
...
class Layers48MMSPairMixerEncoder(PairMixerEncoder):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.encoder_tag = (
            self.encoder_tag + f"_layers{LAYER_IDS[0]}_{LAYER_IDS[1]}_mms"
        )
    ...
    def _forward_graphs(self, graphs):
        ...
        with torch.no_grad():
            for batch in tqdm(
                ...
                desc=f"GNN forward (concat[{LAYER_IDS[0]},{LAYER_IDS[1]}]+max,mean,std)",
                ...
                z = torch.cat([z_max, z_mean, z_std], dim=-1)         # (G, 576)
...
def main() -> None:
    ...
    _REGISTRY["pairmixer"] = (__name__, "Layers48MMSPairMixerEncoder")

    out_csv = ROOT / "results" / "admet_layers48_mms_oneshot.csv"
    cache_dir = (
        ROOT / "datacache"
        / f"pairmixer_probe_fps_layers{LAYER_IDS[0]}_{LAYER_IDS[1]}_mms"
    )
    ...
    sys.argv = [
        ...
        "--ckpt-tag", f"4ds_ep099_layers{LAYER_IDS[0]}_{LAYER_IDS[1]}_mms",
        "--pretrain-label",
        f"4ds-ep099-layers{LAYER_IDS[0]}_{LAYER_IDS[1]}-mms-mmhp",
