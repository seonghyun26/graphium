#!/usr/bin/env python
"""Stage 5 (ESM-C): Filter and normalize the merged DTI-ESMC dataset.

Applies protein-stratified sampling and SMILES diversity filtering to reduce
the ~1.62M-row dataset to 100,000 rows. Then applies L2 and Z-score
normalization.

Filtering strategy (seed=42, target=100,000):
    Phase 1: Sample 1 random row per protein (~5,222 rows).
             Guarantees every protein is represented at least once.
    Phase 2: From the remaining rows, select ~94,778 rows whose SMILES
...
Final output files (copied to data/dti-processed/):
    - dti_esmc_100k.csv               : SMILES_nometa + 1152 feature columns
    - dti_esmc_100k.parquet            : same as CSV, compressed
    - dti_esmc_100k_norm_stats.pt      : mean/std vectors for denormalization

Intermediate output files (in --output-dir):
    - tdcdti-esmc-filtered.parquet     : filtered (raw embeddings, no norm)
    - tdcdti-esmc-filtered-l2.parquet  : L2-normalized embeddings
    - tdcdti-esmc-filtered-zscore.*    : Z-score normalized embeddings

Usage:
    python 05_filter_normalize_esmc.py \\
        --input graphium/data/dti/tdcdti-esmc.parquet \\
        --output-dir graphium/data/dti \\
        --final-dir graphium/data/dti-processed \\
        --seed 42 --target-size 100000
"""
...
def normalize_zscore(
    ...
    """
    mean = embeddings.mean(axis=0)  # shape [1152]
    std = embeddings.std(axis=0)    # shape [1152]
...
def save_parquet(df: pd.DataFrame, embeddings: np.ndarray, path: str):
    """Save as parquet: protein_id, SMILES, feature_0..feature_1151."""
...
def save_final_csv(df: pd.DataFrame, embeddings: np.ndarray, path: str):
    """Save final CSV with SMILES_nometa + feature columns (matches ESM-2 format)."""
    out_df = pd.DataFrame(embeddings, columns=FEATURE_COLS)
...
def main():
    parser = argparse.ArgumentParser(
        description="Stage 5 (ESM-C): Filter and normalize the DTI-ESMC dataset"
    )
    parser.add_argument(
        "--input",
        type=str,
        default="graphium/data/dti/tdcdti-esmc.parquet",
        help="Input parquet from Stage 4 (default: graphium/data/dti/tdcdti-esmc.parquet)",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    print("Stage 5 (ESM-C): Filter + Normalize DTI-ESMC dataset")
    print("=" * 60)
    ...
    filtered_path = os.path.join(args.output_dir, "tdcdti-esmc-filtered.parquet")
    ...
    l2_parquet = os.path.join(args.output_dir, "tdcdti-esmc-filtered-l2.parquet")
    l2_pt = os.path.join(args.output_dir, "tdcdti-esmc-filtered-l2.pt")
    ...
    zscore_parquet = os.path.join(
        args.output_dir, "tdcdti-esmc-filtered-zscore.parquet"
    )
    zscore_pt = os.path.join(args.output_dir, "tdcdti-esmc-filtered-zscore.pt")
    ...
    stats_pt = os.path.join(
        args.output_dir, "tdcdti-esmc-filtered-zscore-stats.pt"
    )
    torch.save(
        {
    ...
    check(
        len(df_final) == args.target_size,
        f"Final CSV rows: {len(df_final)} != {args.target_size}",
    )
    check(
    ...
    # ---- Summary ----
    print("\nDone. Final output files:")
    for f in [final_csv, final_parquet, final_stats]:
